...
/*--------------------------------------------------------------------------------------------------
Datei      		 : ErrorTemplateCompiler.php
Beschreibung 	 : Error Tag Template Kompilirungs Klasse
Copyright  		 : Routecms © 2015
Author 		     : Olaf Braun
Letzte Änderung  : 13.01.2015 Olaf Braun
-------------------------------------------------------------------------------------------------*/

class ErrorTemplateCompiler extends TemplateCompiler {

	/**
	 * @see TemplateCompiler::compileTag()
	 */
	public function compileTag() {
		$result = '<?php if(isset($this->vars["errorInput"]) ';
		...
		for($i = 0, $j = count($matches[1]); $i < $j; $i++) {
			...
			if($name == "var") {
				...
				$result .= '&& $this->vars["errorInput"] == '.$value.'';
